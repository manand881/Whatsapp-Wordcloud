...
from os.path import isfile                  # importing isFile & join to detect if the items in the directory are files
from os.path import join
from os import listdir                      # importing listdir to list items in the directory

import matplotlib.pyplot as plt             # importing matplotlib to process wordcloud data into an image
import os                                   # importing os to get current working directory

chatfile_names = list()                     # empty files to append chat file names
mypath = os.getcwd()                        # get current working directory path
...
for names in chatfile_names:  # iterates through all the chat files
    ...
    for line in f:  # iteraties through all the lines
        ...
        colon1_pos = reader.find(":",
                                 13)  # assigns colon and dash position as name is located between these two in chat format
    ...
    path = os.path.join(location,
                        name_of_chat[:-4])  # adding the name of the chat minus the extension to generate a folder

    try:
        os.mkdir(path)  # creating a folder with the name of the chat
        os.replace(os.path.join(location, name_of_chat),
                   os.path.join(path, name_of_chat))  # changing path to enter into the chat folder
    ...
    cloud(first_persons_string, iter_first_persons, first_persons_message,
          first_person_wordcount)  # calling the word cloud function for the first person
    cloud(second_person_string, iter_second_person, second_persons_message,
          second_person_wordcount)  # calling the word cloud function for the second person
